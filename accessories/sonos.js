...
SonosAccessory.prototype = {
    ...
    sonos.search(function(device) {
      ...
      device.deviceDescription(function (err, description) {
        ...
        if (description["zoneType"] == '3') {
          console.log("Found playable device");
  ...
  setPlaying: function(playing) {
    ...
    if (!this.device) {
      console.log("No device found (yet?)");
      return;
    }
    ...
    if (playing) {
      this.device.play(function(err, success) {
        console.log("Playback attempt with success: " + success);
      });

      if (this.playVolume) {
        this.device.setVolume(this.playVolume, function(err, success) {
          if (!err) {
            console.log("Set volume to " + that.playVolume);
          }
          else {
            console.log("Problem setting volume: " + err);
          }
        });
      }
    }
      ...
      this.device.stop(function(err, success) {
        console.log("Stop attempt with success: " + success);
      });
    }
  },
